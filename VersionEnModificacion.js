...
Qualtrics.SurveyEngine.addOnload(function()
{
    ...
    q.find("input[type=text]").on("paste touchend", function(event) {
        var pastedData = event.originalEvent.clipboardData.getData('text').replace(/\D/g, '');
...
function validCode(digits, that) {
	if(digits.length == 17){
        ...
        if (banderaFechaError == true) {
            ...
            Qualtrics.SurveyEngine.setEmbeddedData('Juliana', fechaJulianaHoy());
            Qualtrics.SurveyEngine.setEmbeddedData('Fecha_P', fecha_actual);
       ...
       // Manejo del mensaje de error
        ...
        var textoError = document.getElementById('error-message');
        if (textoError) {//11510913617017718
            textoError.style.display = 'none';
        }
    }
}
