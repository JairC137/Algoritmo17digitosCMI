...
var numIntentos = 0; // Variable para llevar la cuenta de intentos
...
function validCode(digits, that) {
    ...
    if (digits.length == 17) {
        ...
        if (pais != "1" && pais != "2" && pais != "3") {
            ...
            numIntentos++;
            if (numIntentos >= 2) {
                Qualtrics.SurveyEngine.setEmbeddedData('Algoritmo', 'Algoritmo invalido');
                that.showNextButton();
            } else {
                that.hideNextButton();
            }
            mostrarMensajeError("Verifique que el código sea válido");
        } else {
            pase = true;
            Qualtrics.SurveyEngine.setEmbeddedData('Algoritmo', 'Algoritmo valido');
            that.showNextButton();
